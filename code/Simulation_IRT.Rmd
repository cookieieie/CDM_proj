...
dir.create("../sim_plots_single", showWarnings = FALSE)
```
...
for (str in structure) {
  for (cor in correlation) {
    for (nd in num_dim) {
      ...
      dname <- paste0("../sim_data/data_", str, "_cor", cor, "_dim", nd, "_rep", rep_id, ".csv")
      write.csv(data, dname, row.names = FALSE)
      
      # save true theta
      tname <- paste0("../sim_data/theta_", str, "_cor", cor, "_dim", nd, "_rep", rep_id, ".csv")
...
```{r}
files <- list.files("../sim_data", pattern = "^data_.*\\.csv$", full.names = TRUE)
...
for (f in files) {
  ...
  gdina_RMSEA <- gdina_stats$RMSEA2         # point estimate
  ...
  res_obj <- list(
    ...
    mp = mp,
    theta_hat = theta_hat,
    mirt_fitstats = list(AIC = mirt_fit@Fit$AIC, 
  ...
  # save results to RDS
  fname <- paste0("../sim_results/res_", str,
                  "_cor", cor,
                  "_dim", nd,
                  "_rep", rep_id, ".rds")
...
```{r}
files <- list.files("../sim_results", pattern = "^res_.*\\.rds$", full.names = TRUE)
...
for (f in files) {
  ...
  # remove the probabilities of exactly 0 or 1 --> needed for pearson correlation
  eps <- 1e-5
...
```{r}
write.csv(result_df_rep, "../result_df_rep.csv", row.names = TRUE)
...
all_fit_stats <- lapply(files, function(f) {
  ...
  data.frame(
    ...
    gdina_RMSEA2 = res$gdina_fitstats$RMSEA,
...
```{r}
write.csv(all_fit_stats, "../model_fit_stats.csv", row.names = TRUE)
```
...
for (i in seq_along(groups)) {
  ...
  ggsave(file.path("../sim_plots", fname), plot = p, width = 6, height = 4)
}
```
...
path_spec <- c(
  "../sim_results/res_Simple_cor0_dim5_rep3.rds",
  "../sim_results/res_Simple_cor0_dim5_rep6.rds",
  "../sim_results/res_Simple_cor0_dim5_rep9.rds",
  "../sim_results/res_Simple_cor0.3_dim3_rep4.rds",
  "../sim_results/res_Simple_cor0.3_dim3_rep7.rds",
  "../sim_results/res_Simple_cor0.3_dim3_rep10.rds"
)
...
for (p in path_spec) {
  ...
  for (j in 1:cond$num_dim) {
    ...
    ggsave(file.path("../sim_plots_single", fname), plot = p, width = 6, height = 4)
  }
}
