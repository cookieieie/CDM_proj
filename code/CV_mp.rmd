...
## Apply cv_fit_cdm function to CDM data
```{r}
# define Q-matrix for different conditions and then run cv_fit_cdm
run_cv_fit <- function(cvobj, name){
  ...
  res <- cv_fit_cdm(cvobj, Q, model = "LLM")
  return(res)
}
```
...
```{r}
#res_complex_dim3$data_Complex_cor0_dim3_rep10.csv$metrics
```

```{r}
res_all <- c(res_simple_dim3, res_simple_dim5, res_complex_dim3, res_complex_dim5)

# dimension of metrics_all is 120 * 13, 12 conditions each have 10 replications
metrics_all_cdm <- lapply(names(res_all), function(name) {
  ...
  data.frame(
    ...
    accuracy = res$metrics$accuracy[6],
    precision = res$metrics$precision[6],
    recall  = res$metrics$recall[6],
    specificity  = res$metrics$specificity[6],
    f1_score  = res$metrics$f1_score[6],
    mcc = res$metrics$mcc[6],
    auc  = res$metrics$auc[6],
    pr_auc = res$metrics$pr_auc[6],
    log_loss  = res$metrics$log_loss[6]
  )
}) %>%
  bind_rows()

# group by str, cor, and dim to get summarized metrics of 12 conditions
metrics_all_cdm %>%
  group_by(structure, correlation, dim) %>%
  summarize(
...
```{r}
res_all[["data_Complex_cor0_dim3_rep1.csv"]]
res_all[["data_Complex_cor0_dim3_rep1.csv"]]$metrics$accuracy[6]
res_all[["data_Complex_cor0_dim3_rep1.csv"]]$metrics$precision[6]
```
```{r}
metrics_all_cdm
```
...
# read IRT simulated data and save them into a list called cvdata
...
## Apply cv_fit_cdm function to IRT data
```{r, message=FALSE}
# ~ 7 minutes
set.seed(101)
