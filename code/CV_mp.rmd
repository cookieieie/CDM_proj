...
cdm_data_res_complex_dim5_cor0.3 <- cdm_data_res_complex_dim5[grep("cor0\\.3", names(cdm_data_res_complex_dim5))]
...
for (i in 1:10) {
  ...
  data_i <- cdm_data_res_complex_dim5_cor0.3[[data_name]]
...
results_complex_dim5_cor0.3 <- bind_rows(results_complex_dim5_cor0.3) %>%
...
results_complex_dim5_cor0.3 
```

```{r}
# should be the same results in the summary results table above -> Yes
results_complex_dim5_cor0.3 %>%
  group_by(method) %>%
  summarise(across(all_of(cv_metrics), mean)) %>%
  mutate(method = factor(method, levels = c("mp", "attribute", "theta"))) %>%
  arrange(method)
```
