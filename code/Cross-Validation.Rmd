...
dir.create("../CDM_CV_results_5fold", showWarnings = FALSE)
dir.create("../IRT_CV_results_5fold", showWarnings = FALSE)
```
...
cv_fit_irt <- function(cvdata, model_specification, estimator="MAP",...){
  ...
  for(n in 1:cvdata$kfold){
    ...
    # Fit model
    fit[[n]] <- mirt::mirt(cvdata$cvdata[[n]], model_specification, ...)
...
## Define a function to extract CV results for both CDM & IRT
```{r}
extract_metrics_condition <- function(structure, cor, dim,
  ...
  # extract metrics for all reps × methods
  metrics_df <- bind_rows(lapply(seq_along(res_list), function(i) {
    ...
    bind_rows(lapply(methods, function(m) {
      tibble(
        ...
        replicate = replicate,
        accuracy = res[[m]]$metrics$accuracy[metric_index],
        precision = res[[m]]$metrics$precision[metric_index],
        recall  = res[[m]]$metrics$recall[metric_index],
        specificity  = res[[m]]$metrics$specificity[metric_index],
        f1_score  = res[[m]]$metrics$f1_score[metric_index],
        mcc = res[[m]]$metrics$mcc[metric_index],
        auc  = res[[m]]$metrics$auc[metric_index],
        pr_auc = res[[m]]$metrics$pr_auc[metric_index],
        log_loss  = res[[m]]$metrics$log_loss[metric_index]
      )
    }))
  }))
...
```{r}
test <- readRDS("../CDM_CV_results/res_Complex_cor0_dim3_rep1.rds")
test$mp$metrics
...
```{r}
cdm_result %>%
  mutate(method = ifelse(method == "irt", "theta", method))
```
...
for (i in names(cvdata_cdm)) {
  ...
  # save each condition separately
  saveRDS(res, file = file.path("../CDM_CV_results_5fold", clean_i))
...
for (i in names(cvdata_irt)) {
  ...
  # save each condition separately
  saveRDS(res, file = file.path("../IRT_CV_results_5fold", clean_i))
...
```{r}
cdm_result_5fold <- bind_rows(lapply(structures, function(S) {
  bind_rows(lapply(cors, function(C) {
    bind_rows(lapply(dims, function(D) {
      extract_metrics_condition(S, C, D,
                                "../CDM_CV_results_5fold",
                                metric_index = 6)
    }))
  }))
}))

irt_result_5fold <- bind_rows(lapply(structures, function(S) {
  bind_rows(lapply(cors, function(C) {
    bind_rows(lapply(dims, function(D) {
      extract_metrics_condition(S, C, D,
                                "../IRT_CV_results_5fold",
                                metric_index = 6)
    }))
  }))
}))
...
```{r}
(cdm_result_5fold <- cdm_result_5fold %>% 
  mutate(data_source = "CDM"))

(irt_result_5fold <- irt_result_5fold %>% 
  mutate(data_source = "IRT"))
```
