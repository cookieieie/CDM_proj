  ...
  for (cor in correlation) {
    for (nd in num_dim) {
      for (rep_id in 1:rep) {
        ...
        # randomly generate gs value between 0.1 and 0.3
        gs_parm <- matrix(runif(1, 0.1, 0.3), nrow(Q), 2)
        ...
        # simulate response data under LLM
        ...
        # save data
        dname <- paste0("../CDM_sim/data/data_", str, "_cor", cor, "_dim", nd, "_rep", rep_id, ".csv")
...
for (f in files) {
  ...
  # extract estimated attribute
  est_attr <- personparm(gdina_fit)
  ...
  # }
  theta_hat <- fscores(mirt_fit, method = "MAP", QMC = TRUE)
...
```{r}
# average correlation results for different conditions
...
write.csv(result_df_rep_12, "../CDM_sim/result_df_rep_12.csv", row.names = TRUE)
write.csv(result_df_rep_48, "../CDM_sim/result_df_rep_48.csv", row.names = TRUE)
```
...
```{r}
path_spec <- c(
  "../CDM_sim/results/res_Simple_cor0_dim3_rep3.rds",
  "../CDM_sim/results/res_Simple_cor0_dim3_rep6.rds",
  "../CDM_sim/results/res_Simple_cor0_dim3_rep9.rds",
  "../CDM_sim/results/res_Simple_cor0_dim3_rep4.rds",
  "../CDM_sim/results/res_Simple_cor0_dim3_rep7.rds",
  "../CDM_sim/results/res_Simple_cor0_dim3_rep10.rds",
  "../CDM_sim/results/res_Complex_cor0.3_dim3_rep1.rds",
  "../CDM_sim/results/res_Complex_cor0.3_dim3_rep5.rds",
  "../CDM_sim/results/res_Complex_cor0.3_dim3_rep6.rds",
  "../CDM_sim/results/res_Complex_cor0.6_dim5_rep2.rds",
  "../CDM_sim/results/res_Complex_cor0.6_dim5_rep8.rds",
  "../CDM_sim/results/res_Complex_cor0.6_dim5_rep9.rds"
)
...
simmv <- simGDINA(N, Q,
                  gs.parm = gs, 
                  att.dist = "mvnorm", 
                  model = "LLM", # linear logistic model
                  mvnorm.parm = list(mean = m, sigma = vcov, cutoffs = cutoffs))

dat <- GDINA::extract(simmv, what = "dat")
head(dat)
dim(dat)
```
...
<!-- ```{r} -->
<!-- simple_0.1 <- readRDS("../CDM_sim/results/res_Simple_cor0_dim3_rep3.rds") -->
...
```{r}
head(personparm(est, what = "mp"))
# default is what = "EAP"
head(personparm(est))
head(personparm(est, what = "EAP"))
head(personparm(est, what = "MAP"))
```

```{r}
# all possible attribute combinations for 5 attributes, in total 2^5 = 32 combinations
attributepattern(5)
nrow(attributepattern(5)) == 2^5

# the probability of answering each item correctly with 32 different profile combinations
# the second profile only mastered A1, so it only has high probabilities when an item requires A1
est$LC.prob[,2]
est$LC.prob[,32]
```
...
```{r}
profiles <- as.matrix(est_attribute[, 1:5]) # 2000 * 5, # of person * attributes
# to extract with attribute pattern among the total 32 combinations is for each person --> a single number 
# 2000 * 1
gr <- GDINA:::matchMatrix(as.matrix(attributepattern(5)), profiles)
# 
cdmp <- t(est$LC.prob[,gr])
...
```{r}
# stating [, 1:5] is optional, same results
sum(as.matrix(est_attribute[, 1:5]) == profiles)
```
